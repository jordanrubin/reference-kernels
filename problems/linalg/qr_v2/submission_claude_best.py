...
# T1: PARAMETERIZED 2-level blocked Householder QR (codex spine: warp panel + warp inner-apply +
# cuBLAS outer-WY + fast_math), with exposed knobs (NB, IB, inner_method) for per-class autotuning.
# inner_method: 0 = custom warp-apply (codex), 1 = cuBLAS-WY (for shapes where wide GEMM wins).
# (scanner bans the CUDA-queue word everywhere.)
...
torch::Tensor qr2(torch::Tensor Acm, int64_t NB, int64_t IB, int64_t inner_method);
...
__global__ void __launch_bounds__(32) panel_warp(float* A, float* tau, int n, int j, int m, int nb, int b){
    ...
    for(int i=0;i<nb;i++){
        ...
        if(t!=0.0f){
            ...
            for(int r=i+1+lane;r<m;r+=32) As[r+(long)i*n]*=sc;
            if(lane==0) As[i+(long)i*n]=1.0f; __syncwarp();
        ...
        } else if(lane==0){ tm[i]=0.0f; }
        __syncwarp();
    }
}
// inner-apply with the panel reflectors CACHED in shared memory (loaded once per CTA, reused
// across all the columns this CTA owns). One CTA = (matrix, chunk of nW columns); one warp per column.
__global__ void inner_apply_vcache(float* A, const float* tau, int n, int j, int m, int nb, int c0, int nc, int b){
    ...
    float* As=A+(long)bi*n*n+(long)j+(long)j*n; const float* tp=tau+(long)bi*n+j;
    extern __shared__ float sh[];   // Vs[m*nb] reflectors + Cs[nW*m] trailing columns, both cached
    float* Vs=sh; float* Cs=sh+(long)m*nb;
    for(long idx=threadIdx.x; idx<(long)m*nb; idx+=blockDim.x){ int r=idx%m,k=idx/m;
        Vs[r+(long)k*m]=(r<k)?0.0f:(r==k)?1.0f:As[r+(long)k*n]; }
    __syncthreads();
    int cc=chunk*nW+warp;
    if(cc<nc){
        float* Cg=A+(long)bi*n*n+(long)j+(long)(c0+cc)*n;
        float* Cw=Cs+(long)warp*m;
        for(int r=lane;r<m;r+=32) Cw[r]=Cg[r];
        __syncwarp();
        for(int k=0;k<nb;k++){
            float tk=tp[k]; if(tk==0.0f) continue;
            float part=0.0f;
            for(int r=k+lane;r<m;r+=32) part += Vs[r+(long)k*m]*Cw[r];
            float dot=wsum(part);
            for(int r=k+lane;r<m;r+=32) Cw[r] -= tk*Vs[r+(long)k*m]*dot;
        }
        for(int r=lane;r<m;r+=32) Cg[r]=Cw[r];
    }
}
__global__ void small_mtw_solve(const float* G,const float* tau,const float* W1,float* W2,
    ...
    const float* Gi=G+(long)i*sJ; const float* Wi=W1+(long)i*sW+(long)c*jw; float* Wo=W2+(long)i*sW+(long)c*jw;
    float y[8];
    #pragma unroll
    for(int r=0;r<8;r++){ if(r<jw){ float acc=Wi[r];
        #pragma unroll
        for(int p=0;p<8;p++) if(p<r) acc -= Gi[p+(long)r*ldJ]*y[p];
        y[r]=tau[(long)i*n+j0+r]*acc; Wo[r]=y[r]; } }
}
static void wy_update(int b,int n,int j0,int jw,int m,int c0,int nc,int ldJ,long sJ,
    ...
    extract_V<<<((long)b*m*jw+TH-1)/TH,TH>>>(A,V,b,n,j0,jw,m);
...
// Fused whole-matrix QR for small n: ONE block per matrix, matrix resident in shared,
// no cuBLAS, single launch. Same Householder convention as panel_warp (matches geqrf).
__global__ void qr_small_kernel(float* A, float* tau, int n, int b){
    int bi=blockIdx.x; if(bi>=b) return;
    int tid=threadIdx.x, nt=blockDim.x, lane=tid&31, warp=tid>>5, nwarp=nt>>5;
    extern __shared__ float sh[];
    float* sA=sh; float* scr=sh+(long)n*n;
    float* Ag=A+(long)bi*n*n; float* tg=tau+(long)bi*n;
    for(long idx=tid; idx<(long)n*n; idx+=nt) sA[idx]=Ag[idx];
    __syncthreads();
    for(int j=0;j<n;j++){
        float* col=sA+(long)j*n;
        float ps=0; for(int r=j+tid;r<n;r+=nt){ float x=col[r]; ps+=x*x; }
        for(int o=16;o>0;o>>=1) ps+=__shfl_down_sync(0xffffffffu,ps,o);
        if(lane==0) scr[warp]=ps; __syncthreads();
        if(warp==0){ float s=(lane<nwarp)?scr[lane]:0.0f; for(int o=16;o>0;o>>=1) s+=__shfl_down_sync(0xffffffffu,s,o); if(lane==0) scr[0]=s; }
        __syncthreads();
        float sumsq=scr[0]; __syncthreads();
        float alpha=col[j]; float xn2=sumsq-alpha*alpha; float beta,t;
...
torch::Tensor qr_small(torch::Tensor Acm){
    int b=Acm.size(0), n=Acm.size(1);
    auto tau=torch::empty({(long)b,(long)n}, Acm.options());
    int nt=256; size_t shp=((size_t)n*n + (nt>>5))*sizeof(float);
    static bool aset=false;
    if(!aset){ cudaFuncSetAttribute(qr_small_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, 200*1024); aset=true; }
    qr_small_kernel<<<b, nt, shp>>>(Acm.data_ptr<float>(), tau.data_ptr<float>(), n, b);
    return tau;
}

torch::Tensor qr2(torch::Tensor Acm,int64_t NB_,int64_t IB_,int64_t inner_method){
    ...
    static int once=0; if(!once){ cudaFuncSetAttribute(inner_apply_vcache, cudaFuncAttributeMaxDynamicSharedMemorySize, 200*1024); once=1; }
    ...
    const int TH=256; const int GB=(b+TH-1)/TH, ldJ=NB; const long sJ=(long)NB*NB;
    ...
    for(int jo=0; jo<n; jo+=NB){
        ...
        for(int ji=jo; ji<jo+jbo; ji+=IB){
            ...
            panel_warp<<<b,32>>>(A,taud,n,ji,mi,jbi,b);
            int icols=jo+jbo-(ji+jbi);
            if(icols>0){
                if(inner_method==0){
                    int nW=16, cpm=(icols+nW-1)/nW; size_t shp=(size_t)(mi*jbi + nW*mi)*sizeof(float);
                    inner_apply_vcache<<<(long)b*cpm, nW*32, shp>>>(A,taud,n,ji,mi,jbi,ji+jbi,icols,b);
                } else {
                    wy_update(b,n,ji,jbi,mi, ji+jbi,icols,ldJ,sJ, A,taud,V,G,M,T,W1,W2,mPp,iPp,TH);
                }
            }
        }
        int ocols=n-(jo+jbo);
...
_mod = load_inline(name="qr_small_fused", cpp_sources=[CPP_SRC], cuda_sources=[CUDA_SRC],
                   functions=["qr2","qr_small"], extra_cflags=["-O3"], extra_cuda_cflags=["-O3","--use_fast_math"],
                   extra_ldflags=["-lcublas"], verbose=False)

_NB, _IB, _METHOD, _MAXN = 64, 8, 0, 2048   # fixed default = codex's best, to verify parameterization
...
def custom_kernel(data: input_t) -> output_t:
    if (data.is_cuda and data.dtype == torch.float32 and data.dim() == 3
        ...
        if data.size(1) <= 200:
            ...
            tau = _mod.qr2(Acm, _NB, _IB, _METHOD)
        H = Acm.transpose(-2, -1)
        return H, tau
